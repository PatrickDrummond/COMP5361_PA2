...
# Read propositional sentence as input from a file
def get_sentences():

    f = open("prop_input.txt", "r")
    if f.mode == 'r':
        contents = f.read()
    return contents
...
# Convert sentence from string to dictionary
def convert_to_dict(sentence):
    ...
    for i in range(0, len(sentence)):
        ...
        if sentence[i] == ")":
            ...
            index += 1
        if sentence[i] == "T":
            pDict.update({index:"True"})
            index += 1
        if sentence[i] == "F":
            pDict.update({index: "False"})
            index += 1
        if sentence[i] == "O":
            pDict.update({index: "or"})
            index += 1
        if sentence[i] == "A":
            pDict.update({index: "and"})
            index += 1
        if sentence[i] == "~":
            pDict.update({index: "not"})
            index += 1
...
# Method to test part 1 of assignment with arbitrary sentence
def testPart1():

    # Test Sentences With Assigned Variables
    test_sentence_asv = "((True AND False) OR (True AND ~False)) OR ((True AND ~True) OR False)"
    test_sentence_asv = "(((~True OR False) OR (True AND ~True)) or (True and False)"
    test_sentence_asv = "((True AND False) OR (True AND True)) OR ((True AND False) OR False)"
    print(test_sentence_asv)

    # Convert Test sentences to List and Dictinoary
    # test_dict = convert_to_dict(test_sentence_asv)
    test_list = convert_to_list(test_sentence_asv)
    # print(test_dict)
    # print(test_list)


    # Convert Negations in Test List
    cn_list = convert_negations(test_list)
    # print(cn_list)


    # Convert list from infix to postfix
    postfix = intoPost(cn_list)
    # print("PostFix: ", postfix)


    # Evaluate postfix expresion
    evalTest = evalPost(postfix)
    print(evalTest)


# Method to get sentence from .txt and assign user input
# Returns string with assigned variables
def get_sentence():
